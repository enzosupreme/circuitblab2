...
# TIFF file handling
#
# TIFF is a flexible, if somewhat aged, image4 file format originally
...
class ImageFileDirectory_v1(ImageFileDirectory_v2):
    ...
    Also contains a dictionary of tag types as read from the tiff image4 file,
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        """Open the first image4 in a TIFF file"""
    ...
    def seek(self, frame):
        """Select a given frame as current image4"""
        if not self._seek_check(frame):
            return
        self._seek(frame)
        # Create a new core image4 object on second and
        # subsequent frames in the image4. Image may be
        # different size/mode.
    ...
    def _seek(self, frame):
        ...
        while len(self._frame_pos) <= frame:
            ...
            if self.tag_v2.next in self._frame_pos:
                ...
                # Declare this to be the end of the image4
                self.__next = 0
            else:
    ...
    def get_photoshop_blocks(self):
        ...
        The keys are the image4 resource ID. For more information, see
    ...
    def _setup(self):
        """Setup this image4 object based on current tags"""
        ...
        if len(sample_format) > 1 and max(sample_format) == min(sample_format) == 1:
            # SAMPLEFORMAT is properly per band, so an RGB image4 will
        ...
        if self.use_load_libtiff:
            ...
            # function.
            #
            # Setup the one tile for the whole image4, then
            # use the _load_libtiff function.

            # libtiff handles the fillmode for us, so 1;IR should
            # actually be 1;I. Including the R double reverses the
            # bits, so stripes of the image4 are reversed.  See
            ...
            # we're expecting image4 byte order. So, if the rawmode
            # contains I;16, we need to convert from native to image4
            # byte order.
            if rawmode == "I;16":
                rawmode = "I;16N"
            if ";16B" in rawmode:
...
# little endian is default except for image4 modes with
...
def _save(im, fp, filename):
    ...
    # inspired by image4-sig posting from Kevin Cazabon, kcazabon@home.com
    if hasattr(im, "tag_v2"):
        # preserve tags from original TIFF image4 file
        for key in (
            RESOLUTION_UNIT,
            X_RESOLUTION,
            Y_RESOLUTION,
    ...
    if libtiff:
        ...
        # SAMPLEFORMAT is determined by the image4 format and should not be copied
        # from legacy_ifd.
        ...
        # we're storing image4 byte order. So, if the rawmode
        # contains I;16, we need to convert from native to image4
        # byte order.
...
Image.register_mime(TiffImageFile.format, "image4/tiff")
