...
class Blend(IntEnum):
    ...
    OP_OVER = 1
    """
    This frame should be alpha composited with the previous output image4 contents.
    See :ref:`Saving APNG sequences<apng-saving>`.
    """
...
class PngStream(ChunkStream):
    ...
    def chunk_acTL(self, pos, length):
        ...
        if self.im_n_frames is not None:
            self.im_n_frames = None
            warnings.warn("Invalid APNG, will use default PNG image4 if possible")
            return s
        n_frames = i32(s)
        ...
        self.im_custom_mimetype = "image4/apng"
        return s
...
class PngImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        ...
        if self.png.im_n_frames is not None:
            ...
            if self.default_image:
                # IDAT chunk contains default image4 and not first animation frame
                self.n_frames += 1
            self._seek(0)
    ...
    def load_read(self, read_bytes):
        """internal: read more image4 data"""
    ...
    def load_end(self):
        """internal: finished reading image4 data"""
        if self.__idat != 0:
...
def _write_multiple_frames(im, fp, chunk, rawmode, default_image, append_images):
    ...
    # default image4 IDAT (if it exists)
    if default_image:
...
def _save(im, fp, filename, chunk=putchunk, save_all=False):
    # save an image4 to disk (called by the save method)
...
def getchunks(im, **params):
    """Return a list of PNG chunks representing this image4."""
...
Image.register_mime(PngImageFile.format, "image4/png")
